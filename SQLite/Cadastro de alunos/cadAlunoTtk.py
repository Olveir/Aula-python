#cadAlunoTtk.py com a aplicação em Tkinter para fazer o cadastro, utilizando-se de Aluno e AlunoDAO

import tkinter as tk
import tkinter.messagebox
from cadAluno import Aluno, AlunoDAO
...
class Container:
    def __init__(self, parent):
        ...
        self.titulo = tk.Label(self.container, text="Aluno")
        ...
        self.lb_nome = tk.Label(self.container, text="Aluno:")
        ...
        self.lb_numero_faltas = tk.Label(self.container, text="Numero de faltas:")
        self.lb_numero_faltas.config(font=("Arial", 12, "normal", "normal"), foreground="green")
        self.et_numero_faltas = tk.Entry(self.container)
        self.et_numero_faltas.config(font=("Arial", 12, "bold", "bold"), width=5)
        ...
        self.et_nota1.grid(row=3, column=1, columnspan=2, padx=10, pady=10, sticky="w")

        self.lb_nota2.grid(row=4, column=0, padx=10, pady=10, sticky="e")
        self.et_nota2.grid(row=4, column=1, columnspan=2, padx=10, pady=10, sticky="w")

        self.lb_numero_faltas.grid(row=5, column=0, padx=10, pady=10, sticky="e")
        self.et_numero_faltas.grid(row=5, column=1, columnspan=2, padx=10, pady=10, sticky="w")
    ...
    def consultar(self):
        ...
        if idt.isdigit() and idt != "0":
            ...
            if a.idt == 0:
                ...
                self.et_numero_faltas.insert(0, a.numero_faltas)
                self.et_nome.focus()
        else:
    ...
    def limpar(self):
        for entry in (self.et_idt, self.et_nome, self.et_nota1, self.et_nota2, self.et_numero_faltas):
    ...
    def vazio(self):
        if self.et_nome.get() == "":
            tkinter.messagebox.showerror("Nome Obrigatório", "Insira o nome do aluno")
            self.et_nome.focus()
            return True

        if self.et_nota1.get() == "" or (not self.et_nota1.get().isnumeric()):
            tkinter.messagebox.showerror("1º Nota inválida", "Insira uma nota válida")
            self.et_nota1.focus()
            return True

        if self.et_nota2.get() == "" or (not self.et_nota2.get().isnumeric()):
            tkinter.messagebox.showerror("2º Nota inválida", "Insira uma nota válida")
            self.et_nota2.focus()
            return True

        if self.et_numero_faltas.get() == "" or (not self.et_numero_faltas.get().isdigit()):
            tkinter.messagebox.showerror("Numero de faltas inválido", "Insira uma quantidade válida")
            self.et_numero_faltas.focus()
            return True
    ...
    def salvar(self):
        ...
        if self.et_idt.get() == "0" or self.et_idt.get() == "":
            a = Aluno(nome=self.et_nome.get(), nota1=self.et_nota1.get(), nota2=self.et_nota2.get(),
                      numero_faltas=self.et_numero_faltas.get())
            self.dao.incluir(a)
            self.novo()
            ...
            if idt.isdigit():
                idt_int = int(idt)
                a = Aluno(idt=idt_int, nome=self.et_nome.get(), nota1=self.et_nota1.get(), nota2=self.et_nota2.get(),
                          numero_faltas=self.et_numero_faltas.get())
                self.dao.alterar(a)
